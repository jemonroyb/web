...
class ListaPersoneroMesas extends TPage
{
	...
	*/
	public function __construct()
	{
		...
		$this->form->addQuickField('Grupo', $grupo,  '100%' /*, new TRequiredValidator */);
		...
		$column_total_region    = new TDataGridColumn('total_region', 'Ismodes', 'center');
		$this->datagrid->addColumn($column_total_region);
		$column_total_consejero = new TDataGridColumn('total_consejero', 'LLica', 'center');
		...
		*/
		...
		$container->add(TPanelGroup::pack('Mesas asignadas', $this->form));
	...
	*/
	...
	{
		if($object->estado != 'Enviado' || $object->mesa->local->id_coord_local == TSession::getValue('userid') || TSession::getValue('userid')=='4'){
			return TRUE;
		}
		return FALSE;
	}
	...
	*/
	public function onSearch()
	{
		...
		if($data->grupo){
			// creates a filter using what the user has typed
			$filter = new TFilter('grupo', '=', "{$data->grupo}");

			// stores the filter in the session
			TSession::setValue('ViewPersoneroMesa_grupo', $data->grupo);
			$filters[] = $filter;
		}
	...
	public function onReload($param = NULL)
	{
		try
		{
			...
			if(TSession::getValue('userid') != '4')
			{
				...
				$criteria2->add(new TFilter('id_personero','=',TSession::getValue('userid')),TExpression::OR_OPERATOR);
				$criteria2->add(new TFilter('id_coord_local','=',TSession::getValue('userid')),TExpression::OR_OPERATOR);
