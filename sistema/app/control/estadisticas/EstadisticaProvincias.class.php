<?php
/**
* EstadisticaProvincias Listing
* @author  Renán Darío Gonzales Apaza
*/
class EstadisticaProvincias extends TPage
{
	private $form; // form
	...
	*/
	public function __construct()
	{
		...
		$column_departamento = new TDataGridColumn('id_provincia', 'ID', 'left');
		$this->datagrid->addColumn($column_departamento);


		$column_provincia    = new TDataGridColumn('des_lista', 'Provincia', 'left');
...
/*
		...
		$container->add(TPanelGroup::pack('Votos para alcaldes provinciales', $gridpack));
	...
	*/
	public function onShowDetail( $param )
	{
		...
		try
		{
			...
			$column_provincia    = new TDataGridColumn('des_lista', 'Distrito (votos para provincia)', 'left');
...
mesas
LEFT JOIN locales ON mesas.id_local = locales.idlocal
LEFT JOIN listas ON locales.id_provincia = listas.idlista
WHERE
locales.id_provincia IS NOT NULL
GROUP BY
locales.id_provincia
');
