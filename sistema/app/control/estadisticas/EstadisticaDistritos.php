<?php
/**
* EstadisticaDistritos Listing
* @author  Renán Darío Gonzales Apaza
*/
class EstadisticaDistritos extends TPage
{
	private $form; // form
	...
	*/
	public function __construct()
	{
		...
		$column_provincia    = new TDataGridColumn('distrito', 'Distrito', 'left');
...
/*
		...
		$action_edit->setField('id_provincia');*/
		...
		//$this->datagrid->disableDefaultClick();
		$this->datagrid->setGroupColumn('provincia', ' <b> {provincia}</b> : solo votos distritales');
...
/*
		...
		$container->add(TPanelGroup::pack('Votos para alcaldes <b>distritales</b>', $gridpack));
	...
	*/
	public function onShowDetail( $param )
	{
		...
		try
		{
			...
			// get row position
			$pos         = $this->datagrid->getRowIndex('id_provincia', $param['key']);
			...
			$column_provincia    = new TDataGridColumn('des_lista', 'Distrito', 'left');
			...
			$sth                 = $conn->prepare('SELECT
				listas.des_lista,
				Sum(mesas.total_provincia) AS total,
				locales.id_distrito
				FROM
				mesas
				LEFT JOIN locales ON mesas.id_local = locales.idlocal
				LEFT JOIN listas ON locales.id_distrito = listas.idlista
				WHERE
				locales.id_provincia = '.$param['key'].'
				GROUP BY
				locales.id_distrito');
	...
	*/
	public function onReload($param = NULL)
	{
		try
		{
			...
			$sth    = $conn->prepare('SELECT
locales.id_distrito,
prov.des_lista AS provincia,
dist.des_lista AS distrito,
Sum(mesas.total_distrito) AS total
FROM
mesas
LEFT JOIN locales ON mesas.id_local = locales.idlocal
INNER JOIN listas AS dist ON dist.idlista = locales.id_distrito
INNER JOIN listas AS prov ON prov.idlista = locales.id_provincia
WHERE
