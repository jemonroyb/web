 ...
 */
class ListArticulo extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new TQuickForm('form_search_BlogArticulo');
        $this->form->class = 'tform'; // change CSS class
        $this->form = new BootstrapFormWrapper(new TQuickForm);
        $this->form->style = 'display: table;width:100%'; // change style
        $this->form->setFormTitle('ArtÃ­culos');
        ...
        $this->form->addQuickField('Titulo', $titulo, '100%' /*, new TRequiredValidator */ );
        $this->form->addQuickField('Tipo', $tipo, '100%' /*, new TRequiredValidator */ );


        // keep the form filled during navigation with session data
        $this->form->setData(TSession::getValue('BlogArticulo_filter_data'));

        // add the search form actions
        $this->form->addQuickAction(_t('Find'), new TAction(array($this, 'onSearch')), 'fa:search');
        $this->form->addQuickAction(_t('New'), new TAction(array('FormArticulo', 'onEdit')), 'bs:plus-sign green');
        ...
        // define columns
        $column_idarticulo = new TDataGridColumn('idarticulo', 'ID', 'left');
        $this->datagrid->addColumn($column_idarticulo);
        ...
        $column_etiqueta = new TDataGridColumn('etiqueta', 'Etiqueta', 'left');
        $this->datagrid->addColumn($column_etiqueta);
        $column_visible = new TDataGridColumn('visible', 'Visible', 'left');
        $this->datagrid->addColumn($column_visible);


        // create EDIT action
        $action_edit = new TDataGridAction(array('FormArticulo', 'onEdit'));
        $action_edit->setUseButton(false);
        $action_edit->setButtonClass('btn btn-default');
        $action_edit->setLabel(_t('Edit'));
        $action_edit->setImage('fa:pencil-square-o blue fa-lg');
        $action_edit->setField('idarticulo');
        ...
        $action_del->setField('idarticulo');
        ...
        $panel = new TPanelGroup('ArtÃ­culos');
        $panel->add($this->form);
    ...
    public function onReload($param = null)
    {
        try
        {
            ...
            $repository = new TRepository('BlogArticulo');
            $limit = 70;
            // creates a criteria
            ...
            if (TSession::getValue('BlogArticulo_filters_data'))
            {
                foreach (TSession::getValue('BlogArticulo_filters_data') as $filter)
                {
    ...
    public function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('database'); // open a transaction with database
            $object = new BlogArticulo($key, false); // instantiates the Active Record
